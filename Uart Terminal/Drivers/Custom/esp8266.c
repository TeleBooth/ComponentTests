...
ESP8266_STATUS esp8266_reset(UART_HandleTypeDef *huart) {
	...
	int i;
	// it seems like we might lose upwards of 5 characters, so I want to make a wide window
	// starting from 280 to 285 as starting points to look for "ready"
	for(i = 0; i <= 5; i++){
		if(Receive_Wrapper(&responseBuffer[280+i], "ready" , 5) == ESP8266_OK)
			return ESP8266_OK;
	}
	return ESP8266_ERROR;
}
...
ESP8266_STATUS esp8266_connect_TCP(UART_HandleTypeDef *huart, char *ip,
	...
	if(Receive_Wrapper(responseBuffer, R_OK, R_OK_LEN) == ESP8266_ERROR)
		return ESP8266_ERROR;


	// Set IP mode to 1, UART pass-through
	if (esp8266_enable_passthrough(huart) == ESP8266_ERROR)
		return ESP8266_ERROR;
	...
	if(Receive_Wrapper(responseBuffer, R_AT_CIPSEND, R_AT_CIPSEND_LEN) == ESP8266_ERROR)
		return ESP8266_ERROR;

	passthroughMode = 1;
	// abort the uart transfer so that we can resume in passthrough mode gracefully
